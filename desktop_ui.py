...
class DesktopOverlayWindow(QWidget):
    ...
    마우스 투과(Click-through) 및 전역 단축키 Ctrl+Space 제어 기능을 네이티브 Windows API로 제어합니다.
    """
    HOTKEY_ID = 101
    ...
    def init_ui_layout(self):
        ...
        self.input_box.setPlaceholderText("Ctrl + Space로 활성화 / 질문을 입력하세요...")
    ...
    def register_global_hotkey(self):
        """Ctrl + Space 전역 단축키 등록"""
        hwnd = int(self.winId())
        # MOD_CONTROL = 0x0002, VK_SPACE = 0x0020
        success = ctypes.windll.user32.RegisterHotKey(hwnd, self.HOTKEY_ID, 0x0002, 0x0020)
        if success:
            print("[UI] Global Hotkey Ctrl+Space registered successfully.")
        else:
            print("[Error] Failed to register Global Hotkey Ctrl+Space.")
        ...
        if event_type == b"windows_generic_MSG":
            ...
            if msg.message == 0x0312:  # WM_HOTKEY
                if msg.wParam == self.HOTKEY_ID:
                    print("[Event] Ctrl+Space Hotkey detected!")
...
if __name__ == "__main__":
    ...
    print("\n=== 바탕화면 투명 에이전트 창 구동 중 ===")
    print("- 단축키: Ctrl + Space (마우스 투과 해제 및 텍스트 박스 포커싱)")
