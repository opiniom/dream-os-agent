...
class DesktopOverlayWindow(QWidget):
    ...
    마우스 투과(Click-through) 및 전역 단축키 Ctrl+Shift+Space 제어 기능을 네이티브 Windows API로 제어합니다.
    """
    HOTKEY_ID = 101
    ...
    def init_ui_layout(self):
        ...
        self.input_box.setPlaceholderText("Ctrl + Shift + Space로 활성화 / 질문을 입력하세요...")
    ...
    def center_and_position_window(self):
        ...
        print("[UI] [★단축키 안내★] 키보드 단축키 'Ctrl + Shift + Space'를 누르면 바탕화면의 투명 창이 활성화되어 타이핑할 수 있습니다. 'ESC'를 누르면 다시 투과 모드로 돌아갑니다.")
    ...
    def register_global_hotkey(self):
        """Ctrl + Shift + Space 전역 단축키 등록"""
        hwnd = int(self.winId())
        # MOD_CONTROL = 0x0002, MOD_SHIFT = 0x0004, VK_SPACE = 0x0020 (조합 = 0x0006)
        success = ctypes.windll.user32.RegisterHotKey(hwnd, self.HOTKEY_ID, 0x0002 | 0x0004, 0x0020)
        if success:
            print("[UI] Global Hotkey Ctrl+Shift+Space registered successfully.")
        else:
            print("[Error] Failed to register Global Hotkey Ctrl+Shift+Space.")
...
if __name__ == "__main__":
    ...
    print("\n=== 바탕화면 투명 에이전트 창 구동 중 ===")
    print("- 단축키: Ctrl + Shift + Space (마우스 투과 해제 및 텍스트 박스 포커싱)")
